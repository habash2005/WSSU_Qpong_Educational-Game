...
class Ball(pygame.sprite.Sprite):
    ...
    def update(self, classical_computer, quantum_computer):
        self.rect.x += self.velocity[0] #every second, 60 pixels
        self.rect.y += self.velocity[1] #every second, 120 pixels

        if self.rect.y < 0 or self.rect.y > globals.FIELD_HEIGHT - globals.WIDTH_UNIT: #above the screen, first half
